...
def test_withdraw_does_not_leave_debt_under_floor(
    ...
    # We took some debt and deposited into yvMIM
    assert yvault.balanceOf(strategy) > 0

    # Send profits to yVault
    mim.transfer(yvault, yvault.totalAssets() * 0.03, {"from": mim_whale})
    ...
    # Almost all yvMIM shares should have been used to repay the debt
    # and avoid the floor
...
def test_deposit_after_ceiling_reached_should_not_mint_more_mim(
    ...
    ratio_before = strategy.getCurrentCollateralRatio()
    ...
    assert investment_before >= yvault.balanceOf(strategy)
    assert ratio_before < strategy.getCurrentCollateralRatio()


# Fixture 'amount' is included so user has some balance
def test_withdraw_everything_cancels_entire_debt(
    vault, strategy, token, token_whale, user, amount, yvault, mim, mim_whale, gov,
):
    ...
    # Send profits to yVault
    mim.transfer(yvault, yvault.totalAssets() * 0.00001, {"from": mim_whale})
...
def test_withdraw_under_floor_without_funds_to_cancel_entire_debt_should_fail(
    vault, strategy, token, token_whale, gov, yvault, abracadabra
):
    ...
    lower_rebalancing_bound = (
        strategy.collateralizationRatio() - strategy.rebalanceTolerance()
    )
    min_floor_in_band = (
        token_floor * lower_rebalancing_bound / strategy.collateralizationRatio()
    )
    ...
    yvault.transfer(
        token_whale, yvault.balanceOf(strategy) * 0.01, {"from": strategy}
    )
...
def test_tend_trigger_with_debt_under_dust_returns_false(
    vault, strategy, token, token_whale, gov, abracadabra
):
    price = abracadabra.exchangeRate()
    floor = Wei("4_990 ether")  # assume a price floor of 5k as in ETH-C
...
def test_tend_trigger_without_more_mintable_mim_returns_false(
...
def test_tend_trigger_with_funds_in_cdp_but_no_debt_returns_false(
    vault, strategy, token, token_whale, gov, mim, mim_whale, yvMIM
):
    # Deposit to the vault and send funds through the strategy
    token.approve(vault.address, 2 ** 256 - 1, {"from": token_whale})
    ...
    mim.transfer(yvMIM, yvMIM.totalAssets() * 0.005, {"from": mim_whale})
    ...
    assert strategy.balanceOfDebt() == 0
    assert strategy.getCurrentCollateralRatio() / 1e18 > 1000
    assert strategy.tendTrigger(1) == False
